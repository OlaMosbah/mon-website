...
const createSmallCards = (data) => {
	...
	<div class="sm-product">
		...
		<p class="sm-price" data-price="${data.sellPrice}">${data.sellPrice * data.item}LE</p>
		<button class="sm-delete-btn"><img src="./img/close.png" alt=""></
		button>
	</div>
	`;
}
...
const updateBill = () => {
	...
	billPrice.innerHTML = `${totalBill}LE`;
}
	...
	counts.forEach((item, i) => {
		...
		counterMinus[i].addEventListener('click', () => {
			if(item.innerHTML > 1){
				item.innerHTML--;
				totalBill -= cost;
				price[i].innerHTML = `${item.innerHTML * cost}LE`
